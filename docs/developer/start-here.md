...
| **M49** | Agent Debug Loop starts turning one-shot code-agent drafts into budgeted attempts: the agent can read failed checks and repair the draft before a proposal is shown, while post-apply repair remains a new proposal rather than silent code changes. |
