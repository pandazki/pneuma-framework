...
![M3 可部署 substrate：Dev mode 把数据和定义 rows 写入 SQLite app.db，build 产出 release artifact，Docker 挂载 /data/app.db 运行，runtime rediscovery 通过 /api/config 再次暴露 capability](./spec/images/m3-deployable-substrate.zh-CN.png)
...
![M3 release loop：八个站点，从 Dev backend、Migrate app.db、Builder capability、Manifest、Docker image、Volume、Restart 到 Rediscover /api/config；回环表示 restart 回到同一个 mounted app.db](./spec/images/m3-release-loop.zh-CN.png)
...
![M3 证据矩阵：SQLite database、migrations、release manifest、operation API persistence、Builder-created definition row、ledger 与 app_history storage 在 host runtime、release artifact、Docker runtime 和 restart 上的覆盖](./spec/images/m3-evidence-matrix.zh-CN.png)
...
![M3 下一决策门：M3 closed 在中心，推荐分支是 Reference App Prototype；其它分支包括 definition.apply release smoke、deploy adapter、Postgres/vector pressure、hot reload + custom code](./spec/images/m3-next-gate.zh-CN.png)
...
1. [Architecture README](./README.md)：当前地图。
