...
M20 闭合：[ADR-0031](../architecture/adr/0031-open-ended-definition-artifact-boundary.md) 接受这个 v0 边界。M18 里的 routes、sections、style tokens、dynamic modules 是 **Host-owned open-ended artifacts + Host-level approval**，不是 framework definition rows，也不是 `definition.apply_change_set` artifacts。
