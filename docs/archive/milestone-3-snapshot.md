...
![M3 deployable substrate — Dev mode writes data and definition rows into SQLite app.db, build emits a release artifact, Docker runs with a mounted /data/app.db volume, and runtime rediscovery exposes the capability again through /api/config](../architecture/spec/images/m3-deployable-substrate.png)
...
![M3 release loop — eight stations from Dev backend, Migrate app.db, Builder capability, Manifest, Docker image, Volume, Restart, and Rediscover /api/config, with a return arc showing restart returns to the same mounted app.db](../architecture/spec/images/m3-release-loop.png)
...
![M3 evidence matrix — SQLite database, migrations, release manifest, operation API persistence, Builder-created definition row, ledger and app_history storage are checked across host runtime, release artifact, Docker runtime, and restart](../architecture/spec/images/m3-evidence-matrix.png)
...
![M3 next decision gate — M3 closed in the center, with recommended Reference App Prototype branch and secondary branches for definition.apply release smoke, deploy adapter, Postgres/vector pressure, and hot reload plus custom code](../architecture/spec/images/m3-next-gate.png)
