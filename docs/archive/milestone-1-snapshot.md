...
| Agent role | bolt-on chatbox | bolt-on AI features | absent | **first-class** primitive ([ADR-0018](../architecture/adr/0018-operations-as-primitive.md)) |
| Audience | developer / power user | knowledge worker | developer | **Builder** (non-developer) |
| Permission model | imperative RBAC | imperative RBAC | per-app | **declarative DSL with NL bidirectionality** ([ADR-0007](../architecture/adr/0007-permission-dsl.md), [ADR-0008](../architecture/adr/0008-nl-bidirectional.md)) |

Three differentiators that no other framework offers together:

1. **UI binding and Agent tool-call derive from one declaration.** Click a button = call a tool. Same Operation primitive. ([ADR-0018](../architecture/adr/0018-operations-as-primitive.md), [ADR-0023](../architecture/adr/0023-operation-surface-contract.md))
2. **Permission DSL is conversational both ways.** Builder says "only Alice can see"; agent translates to a rule. End-user asks "why can't I see this?"; agent reverse-explains. ([ADR-0008](../architecture/adr/0008-nl-bidirectional.md))
3. **One AST powers filter / policy / trigger.** Learn `WhereClause` once, it covers the stack. ([ADR-0019](../architecture/adr/0019-where-clause-ast.md))
...
![M1 system architecture — definition rows and data rows go through the same Operation pipeline (Operation dispatcher → evaluatePolicy → impact disclosure → handler → app_history + StorageService); definition rows highlighted in sage green as the M1 addition](../architecture/spec/images/m1-system-architecture.png)
...
Builder-authored read Operations default to `public_surface=true` and `view_mountable=true`. Framework governance Operations are explicitly `framework_internal=true`, `public_surface=false`, and `view_mountable=false`, while remaining `agent_callable=true`. See [ADR-0023](../architecture/adr/0023-operation-surface-contract.md).
...
For the current zero-prep team-share package, see [`team-share-demo.md`](../architecture/team-share-demo.md). The old M1-specific talk track has been superseded by the current top-down M19/pre-RC package.
...
**Why:** restart is the current rediscovery boundary. Hiding it would force the demo (and the Builder) to narrate around a black box. Surfacing it as a state snapshot makes the boundary itself a feature. The same channel will eventually carry hot-reload phases without protocol churn. ([ADR-0028](../architecture/adr/0028-framework-event-protocol.md))
...
![M1 ADR coverage radar — 10-spoke radial chart showing 29 ADRs grouped into 10 sections; §3 Permission, §4 Telemetry, and §5 Lifecycle / dual-mode are the three partial spokes (the M2 surface), all others reach 100% coverage](../architecture/spec/images/m1-adr-coverage-radar.png)
...
Suggested workstreams (full list in [`roadmap.md`](../architecture/roadmap.md) §"Stage 5"):
...
2. Read the current [`team-share-demo.md`](../architecture/team-share-demo.md) before the live share.
3. Read [`OPEN-QUESTIONS.md`](../architecture/OPEN-QUESTIONS.md) only after agreeing on the milestone boundary.
4. Read [`roadmap.md`](../architecture/roadmap.md) for the post-M1 phasing.

For deeper architecture, follow [`README.md`](../architecture/README.md) into the ADR set.
