...
![M2 治理链路：Agent 提案、Kernel 边界、Builder 审批、scoped token、framework_system 执行、定义与证据落地](../architecture/spec/images/m2-governance-chain.zh-CN.png)
...
![M2 权力边界：Build Agent 能 propose / validate / explain，Framework Kernel 持有静态 mutation authority 和 scoped token spend，App Policy 只治理 app surface，不能重开 framework internals](../architecture/spec/images/m2-authority-boundary.zh-CN.png)
...
![M2 下一阶段选择：已关闭的 M2 governance chain 位于中心，分叉到四条 hardening 主线：Permission Center 产品化、Protocol hardening、IAM + threat model、Transaction + concurrency](../architecture/spec/images/m2-next-gate.zh-CN.png)
