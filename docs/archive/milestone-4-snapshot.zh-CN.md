...
![M4 product substrate loop：viewer app 调 Operation API，把 inbox_items rows 写入 SQLite app.db，暴露 Data view 和 Live substrate inspector，穿过 Docker restart，并可由 demo runner 重放](../architecture/spec/images/m4-product-substrate-loop.zh-CN.png)
...
![M4 demo surface：同一个运行中的 app 暴露三个可读视角：end-user app、Data view、Live substrate inspector](../architecture/spec/images/m4-demo-surface.zh-CN.png)
...
![M4 evidence matrix：capture、list/status、Data view、substrate inspector 在 Operation API、SQLite reopen、Docker restart、browser UI、demo runner 和 docs 上的覆盖](../architecture/spec/images/m4-evidence-matrix.zh-CN.png)
