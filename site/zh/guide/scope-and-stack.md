...
范围是让 example 保持诚实的东西——小到能端到端地建,真到能演练循环的每一部分。
...
零停机部署,是生产 Host 拥有之物;框架刻意不拥有。*此刻*就把它们点名为范围外,正是阻止
example 悄悄变成一个托管平台的东西。

## 栈:一串 Host 选择的级联

栈里的一切都是**你的**选择。框架不规定框架、不规定数据库、不规定部署目标。所以"选栈"是
